...
def bookings():
    ...
    if request.method == 'POST':
        ...
        if check_out_datetime > check_in_datetime:
            # Check if check-in date is from tomorrow onwards
            ...
            if check_in_date >= tomorrow:
                ...
                # Check-in date is in the past, display an error message
                error_message = 'Invalid check-in date. The check-in date must be from tomorrow onwards.'
                return render_template('bookings.html', error_message=error_message)
        else:
